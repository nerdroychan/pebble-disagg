...
func TestIngestLoad(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/ingest_load", func(td *datadriven.TestData) string {
		...
		case "load":
			...
			meta, _, _, err := ingestLoad(opts, dbVersion, []string{"ext"}, nil, 0, []FileNum{1})
			if err != nil {
				return err.Error()
			}
...
func TestIngestLoadRand(t *testing.T) {
	...
	meta, _, _, err := ingestLoad(opts, FormatNewest, paths, nil, 0, pending)
	require.NoError(t, err)
...
func TestIngestLoadInvalid(t *testing.T) {
	...
	if _, _, _, err := ingestLoad(opts, FormatNewest, []string{"invalid"}, nil, 0, []FileNum{1}); err == nil {
...
func TestIngestLink(t *testing.T) {
	...
	for i := 0; i <= count; i++ {
		t.Run("", func(t *testing.T) {
			...
			err := ingestLink(0 /* jobID */, opts, dir, paths, meta, shared)
			if i < count {
				if err == nil {
					t.Fatalf("expected error, but found success")
				}
	...
	for count := 1; count < 6; count++ {
		b.Run(fmt.Sprintf("memtables=%d", count), func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				...
				b.StartTimer()
				assertNoError(d.Ingest([]string{"ext"}, nil))
			}
		})
	}
}
...
func TestIngestError(t *testing.T) {
	for i := int32(0); ; i++ {
		...
		t.Run(fmt.Sprintf("index-%d", i), func(t *testing.T) {
			...
			inj.SetIndex(i)
			err1 := d.Ingest([]string{"ext0"}, nil)
			err2 := d.Ingest([]string{"ext1"}, nil)
...
func TestIngestIdempotence(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		require.NoError(t, d.Ingest([]string{ingestPath}, nil))
	}
	require.NoError(t, d.Close())
}
...
func TestIngestCompact(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		require.NoError(t, d.Ingest([]string{src(i)}, nil))
	}
...
func TestIngestFlushQueuedMemTable(t *testing.T) {
	...
	ingest := func(keys ...string) {
		...
		require.NoError(t, w.Close())
		stats, err := d.IngestWithStats([]string{"ext"}, nil)
		require.NoError(t, err)
...
func TestIngestFileNumReuseCrash(t *testing.T) {
	...
	for _, f := range files {
		func() {
			...
			err = d.Ingest([]string{fs.PathJoin(dir, f)}, nil)
		}()
		if err == nil || !errors.Is(err, errorfs.ErrInjected) {
	...
	for _, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			runIngest := func(keyVals []keyVal) (et errT) {
				...
				err = d.Ingest([]string{ingestTableName}, nil)
				if err != nil {
		...
		b.Run(op, func(b *testing.B) {
			for _, count := range counts {
				b.Run(fmt.Sprintf("sstables=%d", count), func(b *testing.B) {
					...
					require.NoError(b, d.Ingest(paths, nil))

					{
						...
						require.NoError(b, w.Close())
						require.NoError(b, d.Ingest([]string{broadIngest}, nil))
					}
...
func runBenchmarkManySSTablesIngest(b *testing.B, d *DB, fs vfs.FS, count int) {
	...
	for i := 0; i < b.N; i++ {
		...
		require.NoError(b, w.Set([]byte(n), nil))
		require.NoError(b, w.Close())
		require.NoError(b, d.Ingest([]string{n}, nil))
	}
}
