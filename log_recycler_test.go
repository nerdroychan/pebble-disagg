...
func TestLogRecycler(t *testing.T) {
	...
	require.False(t, r.add(fileInfo{1, 0, false, 1}))
	require.False(t, r.add(fileInfo{2, 0, false, 2}))
	require.False(t, r.add(fileInfo{3, 0, false, 3}))

	// Logs are recycled up to the limit.
	require.True(t, r.add(fileInfo{4, 0, false, 4}))
	require.EqualValues(t, []FileNum{4}, r.logNums())
	require.EqualValues(t, 4, r.maxLogNum())
	fi, ok := r.peek()
	require.True(t, ok)
	require.EqualValues(t, 4, fi.fileNum)
	require.True(t, r.add(fileInfo{5, 0, false, 5}))
	require.EqualValues(t, []FileNum{4, 5}, r.logNums())
	require.EqualValues(t, 5, r.maxLogNum())
	require.True(t, r.add(fileInfo{6, 0, false, 6}))
	...
	require.False(t, r.add(fileInfo{7, 0, false, 7}))
	require.EqualValues(t, []FileNum{4, 5, 6}, r.logNums())
	...
	require.True(t, r.add(fileInfo{7, 0, false, 7}))
	require.EqualValues(t, []FileNum{5, 6}, r.logNums())

	require.True(t, r.add(fileInfo{8, 0, false, 8}))
